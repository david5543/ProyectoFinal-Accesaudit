...
exports.calculateDiscrepancies = calculateDiscrepancies;
...
async function auditPage(url) {
    ...
    const results = await new playwright_2.default({ page }).analyze();
    await browser.close();
    ...
    const wcagCounts = wcagTags.reduce((acc, tag) => ({ ...acc, [tag]: 0 }), {});
    ...
    const conformanceLevel = passRate >= 95 ? 'AAA' : passRate >= 90 ? 'AA' : passRate >= 80 ? 'A' : 'Below A';
    return {
        url,
...
function generateRecommendations(auditReport) {
    ...
    const impactToPriorityMap = {
        'critical': 100,
        'serious': 75,
        'moderate': 50,
        'minor': 25,
    };
    const difficultyToPriorityMap = {
        'fácil': 40,
        'media': 20,
        'difícil': 10,
    };
    // Generar recomendaciones basadas en criterios fallidos
    auditReport.criteria.forEach((criterion, index) => {
...
function findRelatedCriterion(axeRuleId) {
    const criteriaDefinitions = [
        {
            id: '1.1.1',
            level: 'A',
...
function estimateDifficulty(axeRuleId) {
    ...
    if (easyFixes.includes(axeRuleId))
        return 'fácil';
    if (mediumFixes.includes(axeRuleId))
        return 'media';
    if (hardFixes.includes(axeRuleId))
        return 'difícil';
    return 'media';
}
...
function getImpactForCriterion(criterionId) {
    const impactMap = {
        '1.1.1': 'crítico',
        '1.2.1': 'serio',
        '1.3.1': 'moderado',
...
function getDifficultyForCriterion(criterionId) {
    const difficultyMap = {
        '1.1.1': 'fácil',
        '1.2.1': 'difícil',
        '1.3.1': 'media',
...
function getRecommendationForCriterion(criterionId) {
...
function generateActions(axeRuleId, violation) {
    const actionsMap = {
        'image-alt': [
...
function calculateDiscrepancies(criteria, manualResults) {
    const discrepancies = [];
    criteria.forEach((criterion) => {
        const manualResult = manualResults[criterion.id];
        if (manualResult !== undefined) {
            const manualStatus = manualResult ? 'passed' : 'failed';
